...
<div class="card shadow mb-2">
    <div class="card-header py-3">
        <a class="btn btn-success btn-circle btn-lg" href="/processos/add" title="Novo Processo"><i
                class="fas fa-plus"></i></a>
        <?= $this->Html->link(
            '<span class="fas fa-file-excel"></span><span class="sr-only">' . __('xls') . '</span>',
            ['action' => 'xls'],
            ['id' =>'xlsbutton', 'escape' => false, 'title' => __('Obter listagem em formato excel'), 'class' => 'btn btn-primary btn-circle btn-lg float-right']) 
        ?>
        <button id="dynatable-filter" class="btn btn-secondary btn-circle btn-lg float-right mr-2" title="Filtros"><i
...
$(document).ready(function() {
    var writers = {
        ação: function(row) {
            var view = '<a class="btn btn-info mr-1" href="/processos/view/' + row.id +
                '" data-toggle="tooltip" data-placement="top" title="<?=__('Detalhes')?>"><i class="far fa-eye fa-fw"></i></a>'
            var edit = '<a class="btn btn-warning mr-1" href="/processos/edit/' + row.id +
                '" data-toggle="tooltip" data-placement="top" title="<?=__('Edit')?>"><i class="far fa-edit fa-fw"></i></a>'
            var dele = '<a class="btn btn-danger" onclick="return confirm(' + "'Deseja mesmo apagar?'" +
                ')" href="/processos/delete/' + row.id +
    ...
    createDynatable("#dynatable", "/processos/", {
        created: -1
    }, writers);
