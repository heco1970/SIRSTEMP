...
<ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
  ...
  <?php
  if ($this->Acl->hasAccess('/users/index')):
    if ($controller == 'Users' && in_array($action,['index','add'])) {
      $active = 'active';
    }
    $users = $this->Acl->link('<i class="fas fa-fw fa-users"></i> <span>' . __('Users') . '</span>', '/users/index', ['class' => ['nav-link'], 'escape' => false ]);
  ?>
  ...
  <li class="nav-item <?=$active?>" >
    ...
    <div id="collapseAdminMenu" class="collapse <?=$show?>" aria-labelledby="adminMenu" data-parent="#accordionSidebar">
      <div class="bg-white py-2 collapse-inner rounded">
        <?= $acesses?>
        <?= $aclManager?>
      </div>
