...
class AclHelper extends Helper
{
    ...
    public function __construct(View $View, array $config = []) {
        ...
        $this->primaryKey = TableRegistry::get($this->userModel)->getPrimaryKey();
    ...
    private function _aclCheck($url) {
        $userId   = $this->request->getSession()->read('Auth.User.' . $this->primaryKey);
        ...
        $url = Router::parseRequest(new \Cake\Http\ServerRequest(Router::normalize($url)));
        $path = [
            'controllers',
            $url['plugin'],
            $url['controller'],
